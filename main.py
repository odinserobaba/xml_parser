...
pattern_kod_srok_upl = re.compile(r'ПризнСрокУпл="(.*?)"')
...
for line in tqdm(lines, desc="Processing lines", total=len(lines)):
    ...
    try:
        ...
        if inn and kpp:
            ...
            if kod_pok_matches:
                for period,prizn_srok,kod_pok, prizn_stav, nal_baza, sum_akciz in zip(
                        period,
                        kod_pok_matches,
                        prizn_stav_matches,
                        nal_baza_matches,
...
if df.empty:
    ...
    aggregated_df = df.groupby(['ИННЮЛ', 'КПП','ПризнСрокУпл', 'КодПок', 'ПризнСтав','Период']).agg({'НалБаза': 'size', 'СумАкциз': 'sum'}).reset_index()
    ...
    grouped_sum_df = df.groupby(['ИННЮЛ', 'КПП', 'ПризнСрокУпл','КодПок', 'ПризнСтав']).agg({'СумАкциз': 'sum'}).reset_index()
